...
%% Parameters
img_res = 2^9; % 2^11
seg_px  = 140/2^3;     % flat-to-flat pixels per hex
...
piston_error_rms_m =  rms_val/sqrt(sum(M(:))) / (2*pi) * (lambda);
tilt_error_rms_rads =  2*rms_val*(lambda)/(2*pi)/sqrt(sum(M(:)))/(seg_flat_diam_m/2);
defocus_error_rms_m = 4*sqrt(12)*(f0_m/(seg_flat_diam_m/2))^2*(rms_val*(lambda)/(2*pi))/sqrt(sum(M(:)));
